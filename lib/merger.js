...
Merger.prototype.into = function into (target) {
  ...
  while (this.opts.sources.length)
    merge(this.opts.sources.pop(), target, this.opts)
...
Merger.prototype.excluding = function excluding (properties) {
  ...
  for (var i in properties)
    this.opts.exclusions.push(properties[i])
...
function mergeObjects (source, target, opts) {
  ...
  for (property in source) {
    ...
    if (~ opts.exclusions.indexOf(property))
      continue

    item = source[property]

    // Is this an object? Should we perform recursive merge?
    if (opts.recurse && isObject(item)) {
